...
- The deprecated-configuration bridge carries a resolver list, not a decision to trust the caller. An installation upgrading with `artisanpack.analytics.multi_tenant.enabled` on still gets `HeaderResolver` prepended to the shared chain ahead of `DomainResolver`, exactly as before — and it resolves nothing until the flag above is set, so the upgrade cannot widen the trust boundary on an operator's behalf.
